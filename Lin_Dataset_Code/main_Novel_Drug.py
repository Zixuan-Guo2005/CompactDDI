...
from Task33model_MI2_mol import FusionLayer,GNN1,GNN2
...
def train(train_x,train_y,test_x,test_y,net):
    ...
    for epoch in range(args.epoches):
        ...
        with torch.no_grad():
            ...
            test_y = test_y.to(device)  # +.to(device)
            f_input = list()
    ...
    return test_loss / len(test_y), max(test_list), train_l / len(train_y), sum(train_a) / len(train_a), test_list, max_test_output
...
def main():
    ...
    save_result("../result/", "3all_Lin", result_all)
    save_result("../result/", "3each_Lin", result_eve)
