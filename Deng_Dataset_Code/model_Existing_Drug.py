...
class GNN1(nn.Module):
    ...
    def forward(self, idx):
        ...
        smooth_loss = self.smoother.compute_kl_loss()
...
class FusionLayer(nn.Module):
    def __init__(self, args,description,bio_feature_tensor):
        super().__init__()
        self.fullConnectionLayer = nn.Sequential(
            ...
            nn.Dropout(args.dropout),
            nn.Linear(args.embedding_num * 2, 65)).to(device)
        self.description = description
        self.bio_feature_tensor = bio_feature_tensor


        self.TextAutoencoder = EmbeddingAutoencoder(self.description.shape[1], args.embedding_num)
        ...
        self.TextAutoencoder.eval()
        self.bio_featureAutoencoder.eval()
        self.molAutoencoder.eval()

        self.mutual_info_loss = MutualInformationLoss(args.embedding_num)

        self.BioFeatureMLP = nn.Linear(self.bio_feature_tensor.shape[1], args.embedding_num)
        self.MolMLP = nn.Linear(128, args.embedding_num)
