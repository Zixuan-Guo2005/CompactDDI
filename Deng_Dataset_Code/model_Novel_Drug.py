...
class MLPDecoder(nn.Module):
    ...
    def forward(self, features, latent_vector):
        x = torch.cat([features, latent_vector], dim=-1)
...
class FusionLayer(nn.Module):
    def __init__(self, args,bio_feature_tensor):
        ...
        self.decoder1 = MLPDecoder(
            input_dim=args.embedding_num, latent_dim=args.embedding_num, hidden_dim=128, output_dim=args.embedding_num
        )
        self.decoder2 = MLPDecoder(
            input_dim=args.embedding_num, latent_dim=args.embedding_num, hidden_dim=128, output_dim=args.embedding_num
        )
        self.decoder3 = MLPDecoder(
            input_dim=args.embedding_num, latent_dim=args.embedding_num, hidden_dim=128, output_dim=args.embedding_num
        )
